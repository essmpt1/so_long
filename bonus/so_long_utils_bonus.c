...
/*   Updated: 2024/08/10 13:55:01 by malourei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_putnbr_mlx(t_game *data, long number, int *x, int y)
{
	...
	if (number < 0)
	{
		*x += 7;
		number = -number;
		mlx_string_put(data->mlx, data->win.win, *x, y, 0xFF0000, "-");
	}
	if (number > 9)
	...
	*x += 7;
	str[0] = ((number % 10) + '0');
	str[1] = '\0';
	mlx_string_put(data->mlx, data->win.win, *x, y, 0xFF0000, str);
}
...
void	windows_size(t_game *data)
{
	...
	data->win.height = data->map.height;
	data->win.width = data->map.width;
	if (data->map.width > data->ecra.x && data->map.height > data->ecra.y)
	{
		data->win.width = data->ecra.x;
		data->win.height = data->ecra.y;
	}
